...
namespace OBSWebsocketDotNet
{
  public partial class OBSWebsocket
  {
    ...
    {
      get => WSConnection?.ReconnectTimeout ?? wsTimeout;
      set
      {
        wsTimeout = value;

        if (WSConnection != null) WSConnection.ReconnectTimeout = wsTimeout;
      }
    }
    ...
    /// </summary>
    public bool IsConnected => WSConnection != null && WSConnection.IsRunning;
    ...
    public void Connect(string url, string password)
    {
      ...
      if (WSConnection != null && WSConnection.IsRunning) Disconnect();
    ...
    private void WebsocketMessageHandler(object sender, ResponseMessage e)
    {
      ...
      var msg = JsonConvert.DeserializeObject<ServerMessage>(e.Text);
      var body = msg.Data;
      ...
      switch (msg.OperationCode)
      {
        ...
        case MessageTypes.RequestBatchResponse:
          ...
          if (body.ContainsKey("requestId"))
          {
            ...
            if (responseHandlers.TryRemove(msgID, out var handler))
      ...
      bool waitForReply = true)
    {
      ...
      do
      {
        ...
        message = MessageFactory.BuildMessage(operationCode, requestType, additionalFields, out var messageId);
        if (!waitForReply || responseHandlers.TryAdd(messageId, tcs)) break;
    ...
    {
      var response = SendRequest("GetVersion");
    ...
    {
      var response = SendRequest("GetAuthRequired");
      return new OBSAuthInfo(response);
    }

    /// <summary>
    ///   Authenticates to the Websocket server using the challenge and salt given in the passed <see cref="OBSAuthInfo" />
    ///   object
    /// </summary>
    ...
    protected bool SendIdentify(string password, OBSAuthInfo authInfo = null)
    {
      ...
      catch (ErrorResponseException)
      {
        Disconnected?.Invoke(this,
          new DisconnectionInfo(DisconnectionType.Error, WebSocketCloseStatus.ProtocolError, "Auth Failed",
            string.Empty, new AuthFailureException()));
        Disconnect();
        return false;
      }
      ...
      switch (eventType)
      {
        ...
        case "SceneItemVisibilityChanged":
          SceneItemVisibilityChanged?.Invoke(this, (string)body["scene-name"], (string)body["item-name"],
            (bool)body["item-visible"]);
          break;
        case "SceneItemLockChanged":
          SceneItemLockChanged?.Invoke(this, (string)body["scene-name"], (string)body["item-name"],
            (int)body["item-id"], (bool)body["item-locked"]);
          break;
        ...
        case "TransitionBegin":
          TransitionBegin?.Invoke(this, (string)body["name"], (string)body["type"], (int)body["duration"],
            (string)body["from-scene"], (string)body["to-scene"]);
          break;
        case "TransitionEnd":
          TransitionEnd?.Invoke(this, (string)body["name"], (string)body["type"], (int)body["duration"],
            (string)body["to-scene"]);
          break;
        case "TransitionVideoEnd":
          TransitionVideoEnd?.Invoke(this, (string)body["name"], (string)body["type"], (int)body["duration"],
            (string)body["from-scene"], (string)body["to-scene"]);
          break;
        case "ProfileChanged":
        ...
        case "SceneItemDeselected":
          SceneItemDeselected?.Invoke(this, (string)body["scene-name"], (string)body["item-name"],
            (string)body["item-id"]);
          break;
        case "SceneItemSelected":
          SceneItemSelected?.Invoke(this, (string)body["scene-name"], (string)body["item-name"],
            (string)body["item-id"]);
          break;
        ...
        case "SourceDestroyed":
          SourceDestroyed?.Invoke(this, (string)body["sourceName"], (string)body["sourceType"],
        ...
        case "SourceFilterAdded":
          SourceFilterAdded?.Invoke(this, (string)body["sourceName"], (string)body["filterName"],
            (string)body["filterType"], (JObject)body["filterSettings"]);
          break;
        ...
        case "SourceFiltersReordered":
          ...
          {
            var filters = new List<FilterReorderItem>();
        ...
        case "SourceFilterVisibilityChanged":
          SourceFilterVisibilityChanged?.Invoke(this, (string)body["sourceName"], (string)body["filterName"],
        ...
        case "InputVolumeMeters":
          ...
          {
            var inputs = new List<InputVolumeMeter>();
    ...
    {
      if (!WSConnection.IsStarted) return;
    ...
    protected bool SendReidentify()
    {
      if (WSConnection == null || !WSConnection.IsRunning)
        return
          false; // #TODO check if we are in Identified/Connected state, if we didn't send our Identify yet, we shouldn't send Reidentify now
      ...
      {
        Disconnected?.Invoke(this,
          new DisconnectionInfo(DisconnectionType.Error, WebSocketCloseStatus.ProtocolError, "Reidentify Failed",
            string.Empty, new AuthFailureException()));
        Disconnect();
        return false;
      }
    ...
    /// <summary>
    ///   Triggered when a transition (other than "cut") has ended. Please note that the from-scene field is not available in
    ///   TransitionEnd
    /// </summary>
    ...
    /// <summary>
    ///   These events are emitted by the OBS sources themselves. For example when the media file ends. The behavior depends on
    ///   the type of media source being used.
    /// </summary>
    public event MediaEndedCallback MediaEnded;

    /// <summary>
    ///   These events are emitted by the OBS sources themselves. For example when the media file starts playing. The behavior
    ///   depends on the type of media source being used.
    /// </summary>
    public event MediaStartedCallback MediaStarted;

    /// <summary>
    ///   This event is only emitted when something actively controls the media/VLC source. In other words, the source will
    ///   never emit this on its own naturally.
    /// </summary>
